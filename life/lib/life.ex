defmodule Life do
  ...
  def step(grid, {x, y}, {xw, yh}, up_row, down_row, left_col, right_col,
    ...
    if h * w <= 6 * 6 do
      ...
      :logger.info("First half #{inspect first_half}")
      second_half = Map.take(grid, (for m <- x_mid + 1..xw, do: m))
      :logger.info("Second half #{inspect second_half}")
      upper_left = Enum.map(first_half, fn {k, v} -> {k, Map.take(v, (for m <- y..y_mid, do: m))} end)
                |> Enum.into(%{})
      :logger.info("Upper left #{inspect upper_left}")
      upper_right = Enum.map(first_half, fn {k, v} -> {k, Map.take(v, (for m <- y_mid+1..yh, do: m))} end)
                  |> Enum.into(%{})
      :logger.info("Upper right #{inspect upper_right}")
      lower_left = Enum.map(second_half, fn {k, v} -> {k, Map.take(v, (for m <- y..y_mid, do: m))} end)
                 |> Enum.into(%{})
      :logger.info("Lower left #{inspect lower_left}")
      lower_right = Enum.map(second_half, fn {k, v} -> {k, Map.take(v, (for m <- y_mid+1..yh, do: m))} end)
                  |> Enum.into(%{})
      :logger.info("Lower right #{inspect lower_right}")

      # :logger.info("Up row #{inspect up_row}")
      # :logger.info("Down row #{inspect down_row}")
      # :logger.info("Left col #{inspect left_col}")
      # :logger.info("Right col #{inspect right_col}")

      up_row_1 = Map.take(up_row, (for m <- y..y_mid, do: m))
      # :logger.info("Up row 1 #{inspect up_row_1}")
      up_row_2 = Map.take(up_row, (for m <- y_mid+1..yh, do: m))
      # :logger.info("Up row 2 #{inspect up_row_2}")

      down_row_1 = Map.take(down_row, (for m <- y..y_mid, do: m))
      # :logger.info("Down row 1 #{inspect down_row_1}")
      down_row_2 = Map.take(down_row, (for m <- y_mid+1..yh, do: m))
      # :logger.info("Down row 2 #{inspect down_row_2}")

      left_col_1 = Map.take(left_col, (for m <- x..x_mid, do: m))
      # :logger.info("Left col 1 #{inspect left_col_1}")
      left_col_2 = Map.take(left_col, (for m <- x_mid+1..xw, do: m))
      # :logger.info("Left col 2 #{inspect left_col_2}")
      ...
      :logger.info("Up mid row 1 #{inspect up_mid_row_1}")
      down_mid_row_1 = Map.get(lower_left, x_mid + 1)
      :logger.info("down mid row 1 #{inspect down_mid_row_1}")
      up_mid_row_2 = Map.get(upper_right, x_mid)
      :logger.info("Up mid row 2 #{inspect up_mid_row_2}")
      down_mid_row_2 = Map.get(lower_right, x_mid + 1)
      :logger.info("down mid row 2 #{inspect down_mid_row_2}")

      left_mid_col_1 = Enum.map(upper_left, fn {k, v} -> {k, Map.get(v, y_mid)} end)
                    |> Enum.into(%{})
      :logger.info("left mid col 1 #{inspect left_mid_col_1}")
      left_mid_col_2 = Enum.map(lower_left, fn {k, v} -> {k, Map.get(v, y_mid)} end)
                    |> Enum.into(%{})
      :logger.info("left mid col 2 #{inspect left_mid_col_2}")

      right_mid_col_1 = Enum.map(upper_right, fn {k, v} -> {k, Map.get(v, y_mid + 1)} end)
                    |> Enum.into(%{})
      :logger.info("right mid col 1 #{inspect right_mid_col_1}")
      right_mid_col_2 = Enum.map(lower_right, fn {k, v} -> {k, Map.get(v, y_mid + 1)} end)
                     |> Enum.into(%{})
      :logger.info("right mid col 2 #{inspect right_mid_col_2}")
  ...
  def update(grid, {x, y}, {xw, yh}, up_row,
    ...
    :logger.info("Partial grid: #{inspect grid}")
    grid2 = Enum.map(grid,
    ...
    :logger.info("Full grid: #{inspect grid2}")
    # :logger.info("Full grid: #{inspect grid2}")
